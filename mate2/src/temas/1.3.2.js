...
export async function pregunta(i, code, esImprimible = false) {
  ...
  const Pregunta = `
	  ...
	  <p>${i + 1}.- Calcula el error absoluto si se toma a $V_a=${tlacu.cs(va,sf)}$ como una aproximación de $V_e=${tlacu.cs(ve,6)}$</p>
	  <table>
	  <tr><td>Error absoluto </td><td> <math-field></math-field></td></tr>
		</table>
	  </div>
	`
...
export async function render(container, n, code) {
  window.accionR2P = function(i) {
	...
	let ans = tlacu.cs(Math.abs(va-ve),3)
	console.log(`ans: ${rnumero} rans=${ans}`)
