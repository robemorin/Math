...
function myPolinomio(x_0){
  const X=[1], C=[]
  //console.log(`Construcción del polinomio x=${x_0}`)
  if(Math.random()<1){
    //Polinomio grado 3
    
    for(let i=0;i<2;++i){
      X.push(X[i]*x_0)
      C.push(Math.round(Math.random()*4+1)* (Math.random()<0.5?1:-1) )
    }

    //console.log(`inicial ${tlacu.poli.print(C)} *** ${C} `)

    let dummy=0
    for(let i=0;i<2;++i){ 
      dummy += (i+2)*C[1-i]*X[i+1]
      //console.log(`¿ ${(i+2)}*(${C[1-i]})*x^${i+1} = ${(i+2)*C[1-i]*X[i+1]} ?`)
    }
    //console.log(`f'= ${dummy}+C_1`)
    C.push( (Math.random()<0.5?1:-1) * Math.ceil(Math.random()*3) - dummy )
    dummy = 0

    for(let i=0;i<3;++i){
      dummy += C[2-i]*X[i]*x_0
      //console.log(`¿ ${C[2-i]}*x^${i+1} = ${C[2-i]*X[i]*x_0} ?`)
    }
    //console.log(`f(${x_0})= ${dummy}+C_0`)
    C.push(Math.round(Math.random()*10-5) - dummy)


    //console.log(`C: ${tlacu.poli.print(C)}`)
    return C
