const Ficha = [{
	...
	subtema:[
        {
            Nombre:"Movimientos Rígidos",
            topico:[{
                Nombre:"Fundamentos Teóricos",
                func:function(){
                    let theta1=Math.round(Math.random()*300+10)
                    let p=[Math.round(Math.random()*10-5),Math.round(Math.random()*10-5)]

                    let Pregunta = `
    <div class="ficha">
        <h3>Actividad: Rotaciones en el Plano</h3>
        <p><strong>Tema 2.2.1</strong> (Libro de Spong)</p>

        <div class="parte">
            <p>1. <strong>Matriz de rotación 2D:</strong> Dado un ángulo $\\theta$, la matriz de rotación que gira un punto $(x, y)$ en sentido <strong>antihorario</strong> alrededor del origen es:</p>
            $$
            ...
            \\end{pmatrix}
            $$
            <p><strong>Ejercicio 1: </strong>Demuestra que $R(\\theta)$ es una matriz ortogonal ($R^{-1} = R^T$) y explica con tus palabras que significa geométricamente.</p>
            <h3>Formato de Entrega</h3>
            <ul>
                <li>Manda tu evidencia como una imágen en Teams.</li>
            </ul></div>`
                    return Pregunta
                }
            },{
                Nombre:"Rotaciones en el Plano",
                func:function(){
                    let theta1=Math.round(Math.random()*300+10)
                    let p=[Math.round(Math.random()*10-5),Math.round(Math.random()*10-5)]
    ...
    <div class="ficha">
        ...
        <div class="parte">
            ...
            <p> Pasos para <strong>rotación en el origen:</strong> Para rotar un punto $P(x, y)$:</p>
            <ol type="a">
                <li>Obtener la matriz de rotación $R(\\theta)$.</li>
                <li>Obtener el punto rotado $P'$ mediante $R(\\theta)\\cdot P$.</li>
            </ol>
            <p><strong>Ejercicio 1: </strong> Realiza este ejercicios para $P=(${p})^t$  y $\\theta = ${theta1}$°.</p>
            <p><strong>Ejercicio 2: </strong> Use la siguiente función para comprobar su resultado.</p>

           
            <pre style="border:solid 1px blue"><code>
import numpy as np

    def rotar_punto(P, theta):
        # Matriz de rotación
        R = np.array([
        ...
        <h3>Formato de Entrega</h3>
            <ul>
                <li>Manda captura de pantalla del ejercicio asignado.</li>
                <li>Manda fotografía del ejercicio 1 realizado mano.</li>
                <li>Modifica el programa para validar tu ejecicio anterior, manda código fuente y resultado.</li>
            </ul></div>
    </div>`
                    return Pregunta
                }
            },{
                Nombre:"Rotaciones en el Plano II",
                func:function(){
                    let theta1=Math.round(Math.random()*300+10)
                    let p=[Math.round(Math.random()*10-5),Math.round(Math.random()*10-5)]
    ...
    <div class="ficha">
        ...
        <div class="parte">
            <h3>Parte 3: Rotación de figuras</h3>
            <p>Modifica el ejercicio anterior para que $P$ dibuje una figura de tu elección y usa la matriz de rotación </p>
            <h3>Formato de Entrega</h3>
            <ul>
                <li>Manda código fuente y captura del resultado.</li>
            </ul>
    ...
    Nombre: "Dirección de Marcos y Rotaciones",
    func: function() {
        let theta = Math.round(Math.random() * 300 + 10);
        let marco_original = ["eje x (1,0)", "eje y (0,1)"];
        let marco_rotado = ["eje x' (cosθ, sinθ)", "eje y' (-sinθ, cosθ)"];

        let Pregunta = `
<div class="ficha">
    <h3>Actividad: Matrices de Rotación y Dirección de Marcos</h3>
    <p><strong>Tema 2.2.1</strong> (Libro de Spong)</p>

    <div class="parte">
        <p><strong>Contexto:</strong> Al aplicar una rotación, los ejes del marco de referencia (x/y) también giran. La matriz de rotación:</p>
        $$
        R(\\theta) = \\begin{pmatrix}
        \\cos \\theta & -\\sin \\theta \\\\
        \\sin \\theta & \\cos \\theta
        \\end{pmatrix}
        $$
        <p>describe cómo cambian las direcciones de los ejes:</p>
        <ul>
            <li><strong>Eje x original </strong> se convierte a la primera columna de $R(\\theta)$ </li>
            <li><strong>Eje y original </strong> se convierte a la segunda columna de $R(\\theta)$</li>
        </ul>

        <p><strong>Ejercicio 1:</strong> Para θ = ${theta}°:</p>
        <ol type="a">
            <li>Calcula manualmente las nuevas direcciones de los ejes x' e y'.</li>
            <li>Demuestra que los nuevos ejes son ortogonales (su producto punto es cero).</li>
        </ol>

        <h3>Formato de Entrega</h3>
        <ul>
            <li>Foto de tus cálculos manuales (ejercicio 1a y 1b).</li>
        </ul>
    </div>
</div>`;
        return Pregunta;
