...
export function name() {
  return 'Área de un sector circular (Abierta)';
}
...
export async function pregunta(i, totalPreguntas, esImprimible = false) {
  try {
    ...
    // Calcular área exacta del sector circular
    ...
    const Pregunta = `
      ${i === 0 ? '<h2>Fórmula del área de un sector circular: $$A = \\frac{1}{2} r^2 \\theta$$ (con $\\theta$ en radianes) o $$A = \\pi r^2 \\left(\\frac{\\theta}{360^\\circ}\\right)$$</h2>' : ''}
      <div class="pregunta-abierta" data-r="${r}" data-unidades="${unidades}" data-angulos-posibles="${angulosPosibles}" data-theta="${theta}" style="display: none;">
        <p>${i + 1}.- Determine el área del sector circular sombreado en la siguiente figura. Redondee su respuesta a 3 cifras significativas. <span id="resultado_${i}" name="question"></span></p>
...
export async function render(container, n, code) {
  window.accionR2P = function (i) {
    ...
    const thetaRad = unidades === 'rad' ? parseFloat(angulosPosibles) : (theta * Math.PI) / 180;
    const ansCorrectExact = 0.5 * r * r * thetaRad;
