...
export function name() {
  return 'Medidad de tendencia central (Lista de datos)';
}
...
export async function pregunta(numeroPregunta) {
  try {
    ...
    const n_data = Math.floor(Math.random() * 15) + 15;
    let x = [];
    let text_x = '';
    let start = Math.floor(Math.random() * 100) + 1;
    for(let i=0; i<n_data; i++){
        x.push(start + Math.floor(Math.random() * 10));
        text_x += `${(i===0 ? '' : ',  ')}  $${x[i]}$`;
    }
    ...
    const P = `
    ${numeroPregunta + 1}.- Del siguiente conjunto de datos, determine la ${tipoTendencia}.
    <br><br>
    <center>
    <div>
    ...
    let [mean, median, mode] = tlacu.stat.medTendenciaCentral(x);
